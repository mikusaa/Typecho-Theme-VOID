...
- 开发期间将每项已经发生的用户可见变更及时加入 `change-log.md` 的
  `开发中 | Unreleased` 章节，使其持续、准确地反映当前源码；不要等到发版时才补记，
  也不要为了缩短列表而遗漏仍未发布的改动。
...
- 主题源码、更新日志、Release 标题和安装包使用三段展示版本，例如 `4.0.0`。
- Git 标签沿用本仓库将次版本号和修订号直接拼接的紧凑格式，不加 `v` 前缀；例如展示
  版本 `4.0.0` 对应标签 `4.00`，`3.6.2` 对应标签 `3.62`。未经用户明确决定，
  不要另行改用 `4.0.0`、`v4.0.0` 等标签形式。
- `4.x.x` 中的次版本号和修订号默认各使用一位十进制数字，不新增两位数分量，以保持
  紧凑标签无歧义。现有 `4.x.x` 序列不足以继续编号时，先与用户确认新的版本方案，
  不要自行创建 `4.10.0`、`4.0.10` 或无法唯一还原展示版本的标签。
- `index.php`、`$GLOBALS['VOIDVersion']`、`change-log.md`、Release 标题和安装包文件名
  必须表达同一个展示版本；标签和完整变更比较链接使用对应的紧凑标签。

### Pre-release 与提交历史

- 如果目标是先测试最终版本、确认稳定后原地转为正式版，源码、Release 标题、安装包和
  标签从一开始就使用最终版本；只将 GitHub Release 标记为 Pre-release。不要为此追加
  `-beta.1` 等后缀，也不要创建临时 beta 标签或仅用于切换发布状态的源码提交。
- 测试期间内容未变化时，转正式版只编辑同一个 GitHub Release 的 Pre-release 状态，
  不重新打标签、不重新构建、不新建 Release。若测试期间内容发生变化，必须重新确认
  版本和发布方案，不能静默移动已经公开的标签。
- 发版提交只保留确实需要进入源码历史的改动。用户要求“直接撤回提交”或“只保留某项
  改动”时，不要添加一个反向撤销提交；在用户明确授权改写历史后，从正确基点重建聚焦
  提交，并用针对已核验远端 SHA 的 `--force-with-lease` 更新分支。
- 用户要求先查看规则、文案、差异或发版方案时，停在未提交工作区供其审阅；不要自行
  提交、推送、创建或移动标签、编辑 Release，也不要上传或替换发布资产。

### 发版门禁与安装包

1. 创建 Pre-release 或正式 Release 前，将该发布提交包含的 `Unreleased` 条目全部移入
   对应展示版本和日期的版本段；不得重复保留，也不得把已经随包发布的条目继续标成
   `Unreleased`。未纳入本次发布的后续改动继续准确保留在未发布章节。
2. 确认工作区干净、发布提交已经确定且内容准确；从该确切提交执行 `npm ci` 和
   `make verify`，不能复用之前提交或相同文件树的历史测试结论。
3. 推送源码后，用远端查询确认 `master` 指向预期提交，并等待该 SHA 对应的 GitHub
   Actions `verify`、PHP 7.0/8.5 合同矩阵和 `nightly` 全部成功；流水线仍在运行或失败时
   不得创建 Release。
4. 在确切发布提交上创建注解标签，推送后同时核验标签对象和解引用后的提交 SHA。
5. 只发布同一提交新生成的 `build/` 完整运行单元。ZIP 必须以 `VOID/` 为唯一顶层目录，
   文件名使用 `VOID-X.Y.Z.zip`，不能上传源码目录、GitHub 自动生成的 Source code 或
   来自旧 build 的混合内容。
6. 上传前检查暂存目录与 `build/` 一致，执行 ZIP 完整性测试，并记录本地 SHA-256；
   不能以文件存在、大小相近或旧包可以解压代替这些检查。

### Release note 与发布后核验

- Pre-release 的说明沿用正式版本风格，不写开发流水账，也不使用 beta 版文案。大范围
  重构版本只在最顶部增加简短的“重构版本提示”，随后按需保留兼容性和缓存升级提醒。
- 正文使用面向用户的分类变更条目，例如“新增 / 优化 / 修复 / 重构 / 调整 / 移除”，
  末尾提供从上一正式标签到当前标签的“完整变更”比较链接。
- Release 标题使用展示版本，例如 `Version 4.0.0`；是否使用图标应与相邻正式版本保持
  一致。Release 标签使用紧凑版本，例如 `4.00`，资产名使用 `VOID-4.0.0.zip`。
- 发布后分别核验远端分支 SHA、标签解引用 SHA、Release 的 draft/Pre-release/Latest
  状态、Release note、资产名称、大小和远端摘要；Pre-release 不应抢占 Latest，转为
  正式版后再确认它成为预期的 Latest；远端资产 SHA-256 必须与本地包一致。
- 撤回误发版本时，分别清理错误 Release、发布资产、远端标签和本地标签，再确认旧
  Release 不可访问、旧标签不存在。不要把“命令成功返回”当成远端状态已经正确。
- 交付报告必须把源码提交与推送、CI、标签、GitHub Release、安装包以及本地部署/HTTP
  验证作为独立事实说明，不能用其中一项代替另一项。
