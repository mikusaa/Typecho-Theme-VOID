...
主题设置提供“跟随设备”“定时切换”“日间模式”和“夜间模式”四种颜色模式。新安装默认跟随访客设备；已有站点继续保留原来的定时设置。定时切换使用超高级设置中的 `darkModeTime`，并以访客浏览器的当前时间判断。

前台设置面板中的单按钮依次切换“跟随主题设置”“手动日间”和“手动夜间”。手动选择在当前浏览器会话内有效，重新选择设备图标即可恢复后台设置。旧版超高级设置中的 `followSystemColorScheme: true` 会自动兼容为“跟随设备”。
...
**🍰 2026-08-03 Version 3.5.4.1**

* 新增：[主题颜色] 增加跟随设备模式，并实时响应设备深浅色变化。
* 优化：[主题颜色] 保留定时切换并将跟随设备设为新安装默认值，旧站点配置保持兼容。
* 优化：[主题颜色] 前台单按钮支持跟随设置、手动日间和手动夜间三种状态，可随时恢复自动判断。
* 优化：[主题颜色] 统一颜色状态优先级，支持时间边界实时切换并减少首屏闪烁。
