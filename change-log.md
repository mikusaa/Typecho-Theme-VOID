...
* 优化：[首页封面] 主题编辑器独立探测并机会式保存可信封面尺寸，首页和归档在尺寸可用时按真实比例预留空间，并在封面解码完成后以 `0.5s` 淡入；仅实际显示的封面参与前两张立即加载、其余原生懒加载的优先级，隐藏样式不再产生图片请求。
* 调整：[图片加载] 后台“内容图片懒加载”成为唯一公开开关，普通正文与友链缩略图统一使用浏览器原生懒加载，Gallery 保留分批脚本加载；移除 `browserLevelLoadingLazy`、`bluredLazyload`、`CDNType` 超高级设置，旧配置中的键会被安全忽略。
* 优化：[超高级设置] 将配置样例整理为可直接解析的标准 JSON，并增加字段类型、默认值、作用范围和依赖条件说明。
* 修复：[评论提示] 在保持动态输出安全编码的同时，兼容换行、加粗和经过协议校验的链接，并丢弃其他标签与属性。
* 修复：[VOID 插件] 使用语义版本比较并校验插件类与版本声明，避免多位次版本、预发布版或非法版本被错误判断，同时统一年度归档的能力开关。
...
* 修复：[首页瀑布流] 宽屏首次打开时在 Masonry 接管前保持可见双栏，避免页面先显示单栏再切换为双栏。
* 修复：[首页瀑布流] 页面跨越移动端与桌面端断点时正确启用或销毁 Masonry，避免放大窗口后仍保持单列及缩小窗口后残留定位样式。
* 修复：[PJAX/首页瀑布流] 打开文章时保持旧瀑布流布局直到内容替换，避免双栏短暂回流为单栏。
