...
* 重构：[评论交互] 将评论查询、回复与取消、hash 同步、线程展开收起及分页渲染迁移到原生 DOM 和事件 API，保持 PJAX 生命周期幂等、焦点恢复和滚动位置；评论提交请求暂时保留 jQuery 兼容边界。

**🖼️ 2026-09-04 Version 3.6.2**

> **升级提醒：** 更新主题后，请将主题 `assets` 文件夹中的 `VOIDCacheRule.js` 复制到 Typecho 站点根目录并覆盖旧文件。

* 修复：[正文图片] 恢复 `vwid/vhei` 对正文显示尺寸和加载前占位的控制，避免原生懒加载导致目录定位漂移；PhotoSwipe 改用资源实际像素尺寸计算灯箱缩放，并在原图加载完成后校正未知尺寸。
* 修复：[评论区] 统一评论锚点、PJAX 与线程折叠的滚动协调，避免直达评论后因延迟内容和入场动画再次偏移；展开、翻页和收起均保持当前视觉位置及一致的文字行高，并稳定键盘焦点及回复按钮间距，同时隔离目录折叠样式冲突。
* 重构：[正文图片查看器] 使用 PhotoSwipe 5.4.4 替换手写单图聚焦层，支持组图导航、原尺寸缩放、平移、键盘与移动端手势；保留原图链接、Gallery 分组、裁切缩略图动画、明暗主题、减少动态效果及 PJAX 生命周期，不再通过滚轮关闭查看器。
* 修复：[正文图片查看器] 恢复 PhotoSwipe 原生单击工具栏、双击动态缩放、捏合关闭、放大后边缘切图、三图以上循环及小图点击关闭，并让图片和控件避开移动设备安全区，同时保留原查看器的空格与回车关闭习惯。
* 修复：[Bilibili 播放器] 使用原生宽高比适配正文 iframe，移除旧版桌面控制面板预留高度及重复包裹风险，避免宽屏播放时出现异常黑边。
* 修复：[正文图片] 固定裁切缩略图打开时按原图比例淡入灯箱，避免表格等场景因缩略图与原图比例不同而发生拉伸或异常留白；比例一致的普通图片继续保留原有缩放动画。
* 优化：[正文链接] 使用带下沉距离的低对比度细下划线标识正文链接，并在悬停与键盘聚焦时切换为明暗主题强调色，同时改善长链接换行并保持标题、图片、脚注及卡片组件的独立样式。
* 优化：[正文图片排版] 三图及以上横向图片带移除可见序号，采用浏览器原生惯性与相邻图片吸附；左右按钮改为高对比度的轻量圆形提示并平滑导航相邻图片，同时收紧图片间距、弱化焦点描边，保留桌面拖拽、键盘操作及无 JavaScript 降级。
* 优化：[主图来源] 普通文章、独立页和 Gallery 的题图来源移入题图标题区，显示在作者、日期、评论和阅读量下方；封面文章继续保留原有独立来源布局。
* 优化：[MathJax] 保持公式解析默认关闭；启用后仅在当前 PJAX 内容检测到公式时按需加载 MathJax，并复用全站单次请求、隔离过期 PJAX 排版及支持加载失败后重试。
* 重构：[提示块] 采用 GitHub Alerts 语法和五类明暗主题样式，加入后台编辑器插入菜单；旧 `[notice]` 单段语法继续兼容为 `NOTE`，Feed 清理样式后仍保留可读标题与正文，文章时效提醒继续保持独立语义，评论须知及后台主题提示保持原有样式。
* 优化：[内容时效提醒] 文章提醒改为克制的独立阅读注释，后台开关更名为“显示内容时效提醒”，并以最后更新时间超过 90 天作为显示条件。
* 优化：[后台编辑器] 使用单一 VOID 扩展菜单整合图集、表情和提示块，减少工具栏占用，并保留键盘操作、焦点恢复及明暗主题支持；同时隔离 Typecho 原生工具栏的元素尺寸规则，让窄屏换行正常撑开工具栏并将菜单保持在视口内。
* 重构：[正文表格] 桌面端采用轻量分隔线与固定图片缩略图，移动端将结构明确的普通表格重排为始终完整展开的纵向条目；复杂表格保留可聚焦的横向滚动降级，并改善长链接与中日文换行。

**🍰 2026-08-30 Version 3.6.0**

* 新增：[相册] 增加独立的 `Gallery` 页面模板，按内容分组进行原比例自适应排版和移动端重排，兼容现有 `[photos]` 内容并支持 PJAX 重建、键盘操作、单图聚焦层及无 JavaScript 原图降级；长照片墙按完整行渐进显示，并提供“显示更多照片”按钮。
* 重构：[正文图片] 使用主题语义图片结构与文档坐标单图聚焦层替换 Fancybox，支持点击、`Esc` 和滚动退场，关闭后回到正文原位；保留原图链接、懒加载和无 JavaScript 降级路径。
* 新增：[图片排版] `[photos]` 按图片数量自动采用单图居中、双图等高并排或三图以上横向图片带，支持触屏滑动、桌面拖动和键盘焦点跟随，并修正 iOS 竖图尺寸及宽屏多余空白。
* 优化：[首页封面与图片加载] 编辑器独立探测并机会式保存可信封面尺寸，首页和归档按真实比例预留空间；头图改为高优先级异步解码，首屏封面按实际可见性提高加载优先级并在解码完成后淡入，正确处理缓存命中与失败，隐藏样式不再产生图片请求。
* 调整：[懒加载与超高级设置] 后台“内容图片懒加载”成为唯一公开开关，正文和友链缩略图统一使用浏览器原生懒加载，Gallery 保留分批脚本加载；退役 `browserLevelLoadingLazy`、`bluredLazyload`、`CDNType` 并安全忽略旧配置，同时将样例整理为可直接解析的标准 JSON，补充字段说明。
* 新增：[Feed] 可选仅输出文章正文开头并引导读者前往网站阅读全文，默认关闭；启用后会减少阅读器中的离线全文阅读能力。
* 重构：[赞赏] 迁移至无常驻关闭按钮的原生 Dialog，支持点击二维码、空白区域或按 `Esc` 关闭，补充安全 URL 校验、明暗主题、焦点恢复和 PJAX 生命周期处理。
* 优化：[主题颜色] 新增“跟随设备”模式并实时响应 `prefers-color-scheme`，移除按时间自动切换及 `darkModeTime`；统一固定模式、设备模式和会话手动覆盖，旧 `colorScheme: 0` 自动迁移，首屏提前确定颜色并在减少动态效果时直接切换。
* 优化：[后台编辑器] 为文章自定义字段面板、表单控件和行为开关补充夜间模式，适配 DarkMode 后台主题状态。
* 优化：[友链] 重绘友链卡片的明暗主题、双行标题和键盘焦点状态，并为头像加载失败提供首字母占位。
* 优化：[正文样式] 改进 `details` 折叠内容和文章引用块的日夜间样式、焦点状态、触控区域及段落间距，并为缺少 `summary` 的旧内容自动补充可访问标题。
* 重构：[评论/表情] 使用清单驱动的统一表情选择器替换旧 OwO，评论区与后台编辑器共用表情包数据、短码解析和资源校验；加入 97 个 Bangumi 娘动态表情，支持按可见性加载、静态预览、减少动态效果、省流量模式、独立缓存、最近使用、键盘导航、动态无障碍标签和 PJAX 重建。
* 调整：[评论/表情] 将蛆音娘、哔哩哔哩、米哈游和阿鲁资源统一迁入 `emotes` 包目录，规范文件名大小写并恢复遗漏的“崩坏3_点赞”条目；移除旧 Emoji、泡泡分组和 `::(...)` 短码解析，未知或非法短码保留原文，同时统一入口尺寸并改用 B 站风格图标。
* 修复：[评论] 恢复 Typecho 后台的反垃圾、排序和嵌套层级配置，为 PJAX 评论表单重建安全令牌，避免评论局部模板重复触发全局 header 插件钩子；评论提示支持换行、加粗和经过协议校验的链接，并按白名单丢弃其他标签与属性。
* 修复：[归档] 合法的空搜索、空分类等归档保留正常页面和明确空状态；年度归档修正同秒文章覆盖、跨时区年度错误、密码文章标题泄露和分类统计转义问题，并在 Typecho 1.3 下批量读取分类关系。
* 修复：[PJAX 与首页瀑布流] 生命周期统一由原生 `CustomEvent` 派发并保留旧版 jQuery 参数兼容；自定义重载按事件详情识别目标容器，评论区局部翻页不再误触发主内容脚本；同时修正宽屏首屏闪回、断点双向切换、PJAX 单列回流及 ResizeSensor 残留。
* 优化：[侧边工具条] 与设置面板共享页脚可见高度偏移，并在滚动、尺寸变化和 PJAX 更新后同步，移动端自动复位。
* 调整：[页面滚动] 移除全局滚动条换肤，恢复浏览器原生样式。
* 修复：[安全输出] 按 HTML、JavaScript 和 JSON-LD 上下文序列化动态值，修正搜索路径、微博和 Twitter 分享参数编码、规范链接及空首页结构化数据，保留 Typecho 输出插件钩子并限制字号 Cookie 范围。
* 修复：[错误处理与空站点] 恢复 PHP 错误日志，消除评论未定义变量和 PHP 8.5 反射弃用告警；空内容查询、首页、空归档和 404 不再因站点描述为空而调用文章摘要 API 或读取文章专属字段导致白屏，同时修正 404 页元数据和登录返回地址。
* 修复：[VOID 插件] 使用语义版本比较并校验插件类与版本声明，避免多位次版本、预发布版或非法版本被错误判断，并统一年度归档能力开关。
* 重构：[Service Worker] 使用原生 Cache API 替换停止维护的 `sw-toolbox`，保留离线清单回退、失败响应过滤和缓存容量限制，清理旧缓存及元数据；兼容 Google Fonts 域名缓存，仅注销可验证为 VOID 所有的注册并保守迁移旧版默认注册。
* 优化：[字体加载] 使用 Fontsource 将固定版本的 Open Sans、思源宋体和可选 Fira Code 随主题自托管，按字符范围加载中文字体并提供独立暖缓存，移除无实际用途的 Droid Serif 请求；管理员配置的品牌字体和自定义页头仍保留 Google Fonts 路由兼容。
* 升级：[排版与依赖] pangu 更新至 9.1.0，改进跨节点、链接、斜线、运算符和动态内容的中西文间距并兼容 Safari；同时更新 MathJax 4.1.3、tocbot 4.36.8、littlefoot 4.1.4、ResizeSensor 1.2.3、Sass 1.103.1 等依赖，并随构建包保留第三方许可文件。
* 调整：[工程] CI 和构建基线升级至 Node.js 26，更新 GitHub Actions、nightly 发布 Action、ESLint 配置及开发依赖，修复依赖安全问题并加入表情资源、JavaScript、PHP 契约和构建校验。
