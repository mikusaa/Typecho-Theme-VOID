...
* 优化：[主题颜色] 按时间自动切换继续使用 `darkModeTime` 配置，并在访客浏览器时间跨越起止边界时实时更新。
* 优化：[主题颜色] 统一固定模式、自动模式及访客会话手动覆盖的状态管理，使用独立 Cookie 避免旧自动状态干扰。
* 优化：[主题颜色] 在样式加载前确定初始颜色，减少页面先亮后暗的闪烁。
* 优化：[主题颜色] 设置面板切换按钮改用语义化按钮，补充动态标签、键盘操作及减少动态效果支持。
* 调整：[主题颜色] 移除旧版 `followSystemColorScheme` 隐藏配置，统一由后台主题颜色模式控制，避免多个配置入口造成优先级不明确；升级后需重新选择“跟随设备”。
