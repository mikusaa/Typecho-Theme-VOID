...
* 新增：[后台编辑器] 在 Typecho 1.3+ 文章编辑区下方增加真实首页预览，以只读方式将最新保存的草稿或修订版置于首卡，保留 Typecho 原生文章预览和现有卡片模拟器。
* 优化：[后台编辑器] 首页预览改为与 VOID 编辑器面板一致的独立操作栏，按保存状态直接显示打开、保存并预览或预览上次保存版本，不再使用下拉菜单。
