...
(function () {
    ...
    window['Hypher']['hyphenateElement'] = function (element, language) {
        var hypher = window['Hypher']['languages'][language];

        if (!element || !element.childNodes || !hypher) {
            return;
        }

        for (var i = 0; i < element.childNodes.length; i += 1) {
            if (element.childNodes[i].nodeType === 3) {
                element.childNodes[i].nodeValue = hypher.hyphenateText(element.childNodes[i].nodeValue);
            }
        }
    };
